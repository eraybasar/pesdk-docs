...
1) Download the SDK [here](https://github.com/imgly/pesdk-ios-build/releases/latest), then simply drag `ImglyKit.framework` as well as `PhotoEditorSDK.framework` into the `Frameworks, Libraries, and Embedded Content` section of your target and make sure that the `Embed` settings are set to `Embed & Sign`:
...
![Copy dSYM]({{ site.baseurl }}/assets/images/guides/{{page.platform}}/{{page.version}}/copy-dsym.png)
